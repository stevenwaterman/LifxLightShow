    ...
    companion object {
        fun convert(sounds: SortedSet<out AnalysedSound>,
            ...
            for (sound in sounds) {
                ...
                val brightness = Level(Math.min(1.0, Math.pow((Math.min(0.0, sound.loudnessMax.toDouble()) + 60) / 60, 5.0) * 1.17))

                val color = Color(hue = hue, brightness = brightness)
                val event = Event(Pulse(color, duration, 0.8), lights, start)
                timeline.add(event)
            }
